...
func getTestConfig() *vmwareadapter.Config {
	return &vmwareadapter.Config{
		VCenterURL:         "https://vcenter.test.local/sdk",
		Username:           "test-user",
...
func TestGetResourcePool(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "cluster pool ID",
			poolID:  "vmware-cluster-test-cluster",
			wantErr: true,
		},
		{
			name:    "empty pool ID",
...
func TestUpdateResourcePool(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "update cluster pool",
			poolID: "vmware-cluster-test-cluster",
			pool: &adapter.ResourcePool{
				Description: "Updated cluster pool",
			},
			wantErr: true,
		},
		{
